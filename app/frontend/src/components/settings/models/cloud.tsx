  ...
  const fetchProviders = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(`Nepodařilo se načíst poskytovatele: ${errorData.detail}`);
      }
    } catch (error) {
      console.error('Failed to fetch cloud model providers:', error);
      setError('Nepodařilo se připojit k backend službě');
    }
    setLoading(false);
  };
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-muted-foreground">
            {allModels.length} modelů od {providers.length} poskytovatelů
          </span>
        </div>
        ...
        {loading ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground">Načítání cloudových modelů...</p>
          </div>
        ) : allModels.length > 0 ? (
