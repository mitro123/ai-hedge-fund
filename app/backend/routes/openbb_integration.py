...
async def analyze_portfolio_diversification(request: PortfolioAnalysisRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        analysis = agent.analyze_portfolio_diversification(
...
async def optimize_portfolio(request: PortfolioOptimizationRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if request.objective not in ["max_sharpe", "min_volatility", "max_return"]:
            raise HTTPException(status_code=400, detail="Invalid objective. Use: max_sharpe, min_volatility, or max_return")
...
async def get_company_news(
    ...
    try:
        ...
        news = openbb_provider.get_market_news(
            symbol=symbol,
            limit=limit,
            provider=provider
        )
...
async def get_options_data(
    ...
    try:
        ...
        data = openbb_provider.get_options_data(
            symbol=symbol,
