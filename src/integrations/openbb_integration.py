...
class OpenBBDataProvider:
    ...
    def get_company_info(self, symbol: str, provider: str = "yfinance") -> Optional[Dict[str, Any]]:
        """
    ...
    def get_financial_statements(
        self, symbol: str, statement_type: str = "income", period: str = "annual", limit: int = 5, provider: str = "fmp"
    ) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            if obb is not None and hasattr(obb.equity, "fundamental"):
                if statement_type == "income" and hasattr(obb.equity.fundamental, "income"):
                    result = obb.equity.fundamental.income(symbol=symbol, period=period, limit=limit, provider=provider)
                elif statement_type == "balance" and hasattr(obb.equity.fundamental, "balance"):
                    result = obb.equity.fundamental.balance(
                        symbol=symbol, period=period, limit=limit, provider=provider
                    )
                elif statement_type == "cash" and hasattr(obb.equity.fundamental, "cash"):
                    result = obb.equity.fundamental.cash(symbol=symbol, period=period, limit=limit, provider=provider)
                else:
    ...
    def get_market_news(
        self, symbol: Optional[str] = None, limit: int = 10, provider: str = "benzinga"
    ) -> Optional[pd.DataFrame]:
        """
        Get market news.
    ...
    def get_economic_data(
        self, indicator: str, start_date: Optional[str] = None, end_date: Optional[str] = None, provider: str = "fred"
    ) -> Optional[pd.DataFrame]:
        """
        Get economic data.
        ...
        try:
            if obb is not None and hasattr(obb, "economy"):
                result = obb.economy.fred_series(
                    symbol=indicator, start_date=start_date, end_date=end_date, provider=provider
                )
                return result.to_dataframe() if hasattr(result, "to_dataframe") else None
            else:
    ...
    def get_options_data(
        self, symbol: str, expiration: Optional[str] = None, provider: str = "yfinance"
