...
class OpenBBEnhancedAgent:
    ...
    def analyze_stock_comprehensive(
        ...
        try:
            ...
            income_statement = self.openbb.get_financial_statements(
                symbol, "income", "annual", 3, provider
            )
    ...
    def _analyze_price_data(self, price_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if not avg_gain.empty and not avg_loss.empty and not pd.isna(avg_gain.iloc[-1]) and not pd.isna(avg_loss.iloc[-1]) and avg_loss.iloc[-1] != 0:
