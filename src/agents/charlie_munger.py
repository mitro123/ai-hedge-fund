"""Charlie Munger Agent - Analýza akcií podle principů a mentálních modelů Charlieho Mungera."""
...
def charlie_munger_agent(
    ...
    for ticker in tickers:
        ...
        valuation_analysis = (
            calculate_munger_valuation(financial_line_items, market_cap)
            if market_cap is not None
            else {"score": 0, "details": "No market cap data available"}
        )
...
def analyze_moat_strength(metrics: list, financial_line_items: list) -> dict:
    ...
    r_and_d = [
        getattr(item, "research_and_development", None)
        for item in financial_line_items
        if getattr(item, "research_and_development", None) is not None
    ]

    goodwill_and_intangible_assets = [
        getattr(item, "goodwill_and_intangible_assets", None)
        for item in financial_line_items
        if getattr(item, "goodwill_and_intangible_assets", None) is not None
    ]

    if r_and_d and len(r_and_d) > 0:
        if any(rd is not None and rd > 0 for rd in r_and_d):  # If company is investing in R&D
            score += 1
...
def analyze_predictability(financial_line_items: list) -> dict:
    ...
    if op_income and len(op_income) >= 5:
        ...
        if positive_periods == len(op_income):
            ...
            score += 1
            details.append(
                f"Somewhat predictable operations: Operating income positive in "
                f"{positive_periods}/{len(op_income)} periods"
            )
        else:
            details.append(
                f"Unpredictable operations: Operating income positive in only "
                f"{positive_periods}/{len(op_income)} periods"
            )
    else:
...
def generate_munger_output(
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """Jste AI agent Charlie Munger, který činí investiční rozhodnutí podle jeho principů:

            1. Zaměřte se na kvalitu a předvídatelnost podnikání.
            2. Spoléhejte na mentální modely z více disciplín k analýze investic.
            3. Hledejte silné, trvalé konkurenční výhody (příkopy).
            4. Zdůrazňujte dlouhodobé myšlení a trpělivost.
            5. Ceňte si integrity a kompetence managementu.
            6. Upřednostňujte podniky s vysokými výnosy z investovaného kapitálu.
            7. Plaťte spravedlivou cenu za úžasné podniky.
            8. Nikdy neplatíte příliš, vždy požadujte bezpečnostní rezervu.
            9. Vyhýbejte se složitosti a podnikům, kterým nerozumíte.
            10. "Invertujte, vždy invertujte" - zaměřte se na vyhýbání se hlouposti spíše než na hledání brilantnosti.

            Pravidla:
            - Chvalte podniky s předvídatelnými, konzistentními operacemi a peněžními toky.
            - Ceňte si podniků s vysokým ROIC a cenovou silou.
            - Upřednostňujte jednoduché podniky s pochopitelnou ekonomikou.
            - Obdivujte management s vlastní kůží ve hře a kapitálovou alokací přátelskou k akcionářům.
            - Zaměřte se na dlouhodobou ekonomiku spíše než na krátkodobé metriky.
            - Buďte skeptičtí k podnikům s rychle se měnící dynamikou nebo nadměrným zředěním akcií.
            - Vyhýbejte se nadměrné páce nebo finančnímu inženýrství.
            - Poskytněte racionální, na datech založené doporučení (bullish, bearish nebo neutral).
            ...
            2. Aplikujte alespoň 2-3 konkrétní mentální modely nebo disciplíny k vysvětlení vašeho myšlení
            3. Poskytněte kvantitativní důkazy tam, kde je to relevantní (např. konkrétní hodnoty ROIC, trendy marží)
            4. Citujte, čemu byste se "vyhnuli" ve své analýze (invertujte problém)
            5. Používejte přímý, výstižný konverzační styl Charlie Mungera ve svém vysvětlení

            Například, pokud bullish: "Vysoký ROIC 22% demonstruje příkop společnosti.
            Při aplikaci základní mikroekonomie vidíme, že konkurenti by měli potíže..."
            Například, pokud bearish: "Vidím, že tento podnik dělá klasickou chybu v alokaci kapitálu.
            Jak jsem často říkal o [relevantní Mungerismus], tato společnost se zdá být..."
            """,
            ),
            (
                "human",
                """Na základě následující analýzy vytvořte investiční signál ve stylu Mungera.
    ...
    return cast(CharlieMungerSignal, result)
