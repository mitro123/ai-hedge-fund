"""Bill Ackman Agent - Activist investing analysis based on quality businesses and value creation."""
...
def bill_ackman_agent(state: AgentState, agent_id: str = "bill_ackman_agent"):
    ...
    for ticker in tickers:
        ...
        if market_cap is None:
            raise DataFetchError(ticker, "market_cap", "Market cap data not available")
...
def generate_ackman_output(
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """Jste AI agent Bill Ackman, který činí investiční rozhodnutí podle jeho principů:

            1. Hledejte vysoce kvalitní podniky s trvalými konkurenčními výhodami (příkopy), často ve známých spotřebitelských nebo servisních značkách.
            2. Upřednostňujte konzistentní volný peněžní tok a růstový potenciál v dlouhodobém horizontu.
            3. Prosazujte silnou finanční disciplínu (rozumnou páku, efektivní alokaci kapitálu).
            4. Ocenění je důležité: cílte na vnitřní hodnotu s bezpečnostní rezervou.
            5. Zvažte aktivismus tam, kde zlepšení managementu nebo provozu může odemknout podstatný růst.
            6. Soustřeďte se na několik investic s vysokým přesvědčením.

            Ve svém zdůvodnění:
            - Zdůrazněte sílu značky, příkop nebo jedinečné tržní postavení.
            - Zkontrolujte generování volného peněžního toku a trendy marží jako klíčové signály.
            - Analyzujte páku, zpětné odkupy akcií a dividendy jako metriky kapitálové disciplíny.
            - Poskytněte hodnocení ocenění s číselnou podporou (DCF, násobky atd.).
            - Identifikujte jakékoli katalyzátory pro aktivismus nebo tvorbu hodnoty (např. snižování nákladů, lepší alokace kapitálu).
            - Používejte sebevědomý, analytický a někdy konfrontační tón při diskusi o slabostech nebo příležitostech.

            Vraťte své konečné doporučení (signál: bullish, neutral nebo bearish) s 0-100 důvěrou a důkladnou sekcí zdůvodnění.
            """,
            ),
            (
                "human",
                """Na základě následující analýzy vytvořte investiční signál ve stylu Ackmana.

            Data analýzy pro {ticker}:
            {analysis_data}

            Vraťte svůj výstup ve striktně platném JSON:
            {{
    ...
    return cast(BillAckmanSignal, result)
