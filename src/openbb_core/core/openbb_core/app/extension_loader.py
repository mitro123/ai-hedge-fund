...
class ExtensionLoader(metaclass=SingletonMeta):
    ...
    def __init__(
        ...
        self._obbject_entry_points: EntryPoints = self._sorted_entry_points(group=OpenBBGroups.obbject.value)
        self._core_entry_points: EntryPoints = self._sorted_entry_points(group=OpenBBGroups.core.value)
        self._provider_entry_points: EntryPoints = self._sorted_entry_points(group=OpenBBGroups.provider.value)
    ...
    @staticmethod
    def _get_entry_point(entry_points_: EntryPoints, ext_name: str) -> Optional[EntryPoint]:
    ...
    def obbject_objects(self) -> Dict[str, Extension]:
        ...
        self._obbject_objects = self._load_entry_points(self._obbject_entry_points, OpenBBGroups.obbject)
    ...
    def core_objects(self) -> Dict[str, "Router"]:
        ...
        self._core_objects = self._load_entry_points(self._core_entry_points, OpenBBGroups.core)
    ...
    def provider_objects(self) -> Dict[str, "Provider"]:
        ...
        self._provider_objects = self._load_entry_points(self._provider_entry_points, OpenBBGroups.provider)
    ...
    def _load_entry_points(self, entry_points_: EntryPoints, group: OpenBBGroups) -> Dict[str, Any]:
        ...
        def load_obbject(eps: EntryPoints) -> Dict[str, Extension]:
            ...
            """
            return {ep.name: entry for ep in eps if isinstance((entry := ep.load()), Extension)}
        ...
        def load_core(eps: EntryPoints) -> Dict[str, "Router"]:
            ...
            return {ep.name: entry for ep in eps if isinstance((entry := ep.load()), Router)}
