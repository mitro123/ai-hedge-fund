...
class ReportedFinancialsQueryParams(QueryParams):
    ...
    period: str = Field(default="annual", description=QUERY_DESCRIPTIONS.get("period", ""))
...
class ReportedFinancialsData(Data):
    ...
    period_ending: dateType = Field(description="The ending date of the reporting period.")
    fiscal_period: str = Field(description="The fiscal period of the report (e.g. FY, Q1, etc.).")
    fiscal_year: Optional[int] = Field(description="The fiscal year of the fiscal period.", default=None)

    @model_validator(mode="before")
    @classmethod
    def replace_zero(cls, values):  # pylint: disable=no-self-argument
        """Check for zero values and replace with None."""
        return {k: None if v == 0 else v for k, v in values.items()} if isinstance(values, dict) else values
