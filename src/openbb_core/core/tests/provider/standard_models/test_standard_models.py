...
models_path = (Path(__file__).parent.parent.parent.parent / "openbb_core" / "provider" / "standard_models").resolve()
...
for model_file in model_files:
    ...
    model_module = import_module(f"openbb_core.provider.standard_models.{model_file.stem}")
    for _, obj in inspect.getmembers(model_module):
        if inspect.isclass(obj) and (issubclass(obj, Data) or issubclass(obj, QueryParams)):
...
def test_standard_models(standard_model):
    ...
    for name, field in fields.items():
        assert isinstance(field, FieldInfo), f"Field {name} should be a ModelField instance"
