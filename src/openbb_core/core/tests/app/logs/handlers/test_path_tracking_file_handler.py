...
logging_settings.user_logs_directory.absolute.return_value = Path("/mocked/logs/directory")
...
def test_clean_expired_files(handler):
    ...
    with (
        ...
        patch("openbb_core.app.logs.handlers.path_tracking_file_handler.remove_file_list") as mock_remove_file_list,
    ):
